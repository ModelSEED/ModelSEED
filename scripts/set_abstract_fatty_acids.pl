...
@ARGV == 1 or die("usage: set_abstract_fatty_acids <biochemistry>");

my $auth  = ModelSEED::Auth::Factory->new->from_config;
my $store = ModelSEED::Store->new(auth => $auth);
my $bio_ref= shift @ARGV;
my $bio   = $store->get_object($bio_ref);
die "Unknown biochemistry!" unless defined $bio;

sub process_compound_group {
    my ($grouping) = @_;
    my $abstract_cpd = $grouping->{'group'}->{'grouping'};
    my $real_cpds = $grouping->{'group'}->{'compounds'};
    my $a_cpd = $bio->getObjectByAlias("compounds", $abstract_cpd, "ModelSEED");
    if (!defined $a_cpd) {
	warn "Unable to find compound $abstract_cpd\n";
	next;
    }
    my $abst_uuid = $a_cpd->uuid;
    foreach my $real_cpd (@$real_cpds) {
	my $r_cpd = $bio->getObjectByAlias("compounds", $real_cpd, "ModelSEED");
	if (!defined $r_cpd) {
	    warn "Unable to find compound $real_cpd\n";
	    next;
	}
	print STDERR "Setting abstract compound for $real_cpd to $abstract_cpd\n";
	$r_cpd->abstractCompound_uuid($abst_uuid);
    }
}

sub process_reaction_group {
    my ($grouping) = @_;
    my $abstract_rxn = $grouping->{'group'}->{'grouping'};
    my $real_rxns = $grouping->{'group'}->{'reactions'};
    my $a_rxn = $bio->getObjectByAlias("reactions", $abstract_rxn, "ModelSEED");
    if (!defined $a_rxn) {
	warn "Unable to find reaction $abstract_rxn\n";
	next;
    }
    my $abst_uuid = $a_rxn->uuid;
    foreach my $real_rxn (@$real_rxns) {
	my $r_rxn = $bio->getObjectByAlias("reactions", $real_rxn, "ModelSEED");
	if (!defined $r_rxn) {
	    warn "Unable to find reaction $real_rxn\n";
	    next;
	}
	print STDERR "Setting abstract reaction for $real_rxn to $abstract_rxn\n";
	$r_rxn->abstractReaction_uuid($abst_uuid);
    }
}
...
my $groupObj = { 'grouping' => 'cpd00487', 'compounds' => ['cpd03847','cpd15298','cpd00214','cpd15237','cpd15269','cpd01080','cpd01741','cpd00049','cpd11653','cpd11430','cpd11438','cpd11436','cpd11440','cpd11433','cpd11431','cpd03846','cpd15240','cpd05235','cpd15270']};
process_compound_group({'group' => $groupObj});
...
process_reaction_group({'group' => $groupObj});
...
$groupObj = { 'grouping' => 'cpd12547', 'compounds' => ['cpd12547','cpd15313','cpd15314','cpd15315','cpd15316','cpd15317']};
process_compound_group({'group' => $groupObj});
