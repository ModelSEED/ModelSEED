...
sub _build_idServer { return Bio::KBase::IDServer::Client->new("http://140.221.84.177:7031"); }
1;
...
}elsif(defined($biochem)){
    my $biochemObj = $store->get_object($biochem);
    die "Could not find biochemistry $biochem!\n" unless($biochemObj);
    doBiochemistryAndMapping($biochemObj,'', $kbIdRegister);
}
