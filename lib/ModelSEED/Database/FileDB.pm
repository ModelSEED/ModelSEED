...
sub _construct_anc_desc {
    my ($self) = @_;

    # assume (at this point safely) no merges, deletions, or anything fancy
    # start at alias and work way backwards to build arrays and maps
    my $alias_meta = $self->kvstore->get_metadata('aliases');
    foreach my $alias (keys %$alias_meta) {
        my $ancestors = {};
        my $descendants = {};
        ...
        $descendants->{$uuid} = [];
        while (1) {
            my $obj = $self->kvstore->get_object($uuid);
            my $par = $obj->{ancestor_uuids};
            if (defined($par)) {
                $ancestors->{$uuid} = $par;
                $descendants->{$par->[0]} = [$uuid];
                $uuid = $par->[0];
            } else {
                $ancestors->{$uuid} = [];
                last;
        ...
        my $ancs = _parse_anc_desc_graph($ancestors, $first);
        my $desc = _parse_anc_desc_graph($descendants, $last);

        # save the data to metadata
        foreach my $uuid (keys %$ancs) {
            $self->kvstore->set_metadata($uuid, 'ancestors', $ancs->{$uuid}->{arr});
            $self->kvstore->set_metadata($uuid, 'ancestor_graph', $ancs->{$uuid}->{map});
            $self->kvstore->set_metadata($uuid, 'descendants', $desc->{$uuid}->{arr});
            $self->kvstore->set_metadata($uuid, 'descendant_graph', $desc->{$uuid}->{map});
        }
    }
}

sub _parse_anc_desc_graph {
    my ($graph, $first) = @_;

    my $ret = {};
    while (scalar keys $graph) {
        $ret->{$first} = {
            map => $graph,
            arr => []
        };
        ...
        my $next = $first;
        while (scalar @{$graph->{$next}}) {
            push(@{$ret->{$first}->{arr}}, $graph->{$next}->[0]);
            $next = $graph->{$next}->[0];
        }
        $next = $graph->{$first}->[0];
